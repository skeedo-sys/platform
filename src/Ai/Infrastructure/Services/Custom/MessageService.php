    ...
    public function generateMessage(
        ...
        foreach ($this->llms as $llm) {
            if (in_array($model->value, array_column($llm['models'], 'key'))) {
                ...
                if (($foundModel[0]['config']['tools'] ?? false)) {
        ...
        while ($continue) {
            $resp = $this->client->sendRequest($model, 'POST', '/chat/completions', $body);
            ...
            if (is_null($embeddings) && $message->getAssistant()?->hasDataset()) {
                $embeddings = [];

                foreach ($message->getAssistant()->getDataset() as $unit) {
                    try {
                        $embeddings[] = $this->store->get($unit->getId());
                    } catch (\Throwable $th) {
                        continue;
                    }
                }
